...
export async function videoPitch(
  event: IpcMainInvokeEvent,
  { taskId, inputPath, outputPath, duration, pitch }: AdjustmentArgs
): Promise<string> {
  try {
    ...
    return 'Pitch modifying complete';
  } catch (error) {
    throw new Error(`Processing failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}

export function videoPitchHandler(): void {
  ipcMain.handle('video-pitch', async (event, args: AdjustmentArgs) => {
    try {
      const result = await videoPitch(event, args);
